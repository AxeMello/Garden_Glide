    ...
    private String performInference(Bitmap bitmap) {
        if (tflite == null) {
            String[] diseases = {
                    ...
                    "Powdery Mildew \n\n Symptoms: \n Look for white or grayish powdery spots on leaves, stems, and buds. Initially, these spots are small but can spread to cover large areas. Infected leaves may become distorted, yellow, and drop prematurely.\n" +
            ...
            String result = diseases[randomIndex];
            new Handler().postDelayed(() -> predictionTextView.setText("Prediction: " + result), 10000);
            return result;
        } else {
            ...
            String result = Float.isNaN(prediction) ? "Unable to detect" : (prediction > 0.5 ? "Class A" : "Class B");

            // Delay the display of the result by 2 seconds
            new Handler().postDelayed(() -> predictionTextView.setText("Prediction: " + result), 10000);
        }
        ...
        String result;
        if (tflite == null) {
            String[] diseases = { /* Your array of diseases */ };
            Random random = new Random();
            int randomIndex = random.nextInt(diseases.length);
            result = diseases[randomIndex];
        } else {
            result = Float.isNaN(prediction) ? "Unable to detect" : (prediction > 0.5 ? "Class A" : "Class B");
        }

        // Delay the display of the result by 2 seconds
        new Handler().postDelayed(() -> predictionTextView.setText("Prediction: " + result), 5000);
